    ...
    <header className="header">
      <div className="container-header flex-row align-center">
        ...
        <div>
          {Auth.loggedIn() ? (
            <>
              <Link className="btn btn-dark btn-info m-1 login" to="/me">
                {/* {Auth.getProfile().data.username}'s profile */}
                Profile
              </Link>
              <button className="btn btn-dark m-1 login" onClick={logout}>
                Logout
              </button>
            </>
          ) : (
            <>
              <Link className="btn btn-dark btn-info m-2 login" to="/login">
                Login
              </Link>
            </>
          )}
